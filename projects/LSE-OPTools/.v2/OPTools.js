...
const Config_init = {
    ...
    Config: {
        ...
        Clear_Cmd: "kill @e[type=item]", // 不启用CleanerAPI时的默认清理命令
        Ban_Cmd: {
            // BanGUI功能 命令配置
...
function regCommand() {
    ...
    Cmd.setCallback((cmd, ori, out, res) => {
        ...
        switch (res.action) {
            ...
            case "set":
                if (ori.type == 7) {
                    return out.error("This command can only be executed by players! Do not use this command on the console.");
                }
                if (ori.type !== 0) {
                    return out.error("This command can only be executed by players!");
                }
...
mc.listen("onServerStarted", () => {
    ...
    if (PROFILE_CHECK.old !== PROFILE_CHECK.new)
        logger.error(
            `The configuration file version does not match! \nCurrent version: ${PROFILE_CHECK.old} Latest version: ${PROFILE_CHECK.new}`,
        );
    if (HOME_PAGE_CHECK.old !== HOME_PAGE_CHECK.new)
        logger.error(
            `The homepage file version does not match! \nCurrent version: ${HOME_PAGE_CHECK.old} Latest version: ${HOME_PAGE_CHECK.new}`,
        );
...
function Clear_Item_Ui(pl) {
    if (Config.get("Cleaner_API") !== 1) {
        ...
        const CleanerAPI = {
            /* Cleaner_API导入 */ ReloadCLeaner: ll.import("Cleaner", "ReloadCleaner"),
...
function setMode_Ui(pl) {
    const arr = Array.of(
        Tr("gmode.ui.slider.default"),
        Tr("gmode.ui.slider.0"),
        Tr("gmode.ui.slider.1"),
        Tr("gmode.ui.slider.2"),
        Tr("gmode.ui.slider.3"),
    );
    const Online_Players = Func_Module.GET_ALL_ONLINE_PLAYERS();
    const fm = Func_Module.CustomForm();
...
function setRule_Ui(pl) {
    ...
    pl.sendForm(fm, (pl, id) => {
        ...
        pl.sendModalForm(
            `${PLUGINS_NAME}`,
            `${Tr("grule.ui.modalform.0")}:${gamerule[id].name}\n${gamerule[id].txt}`,
            Tr("grule.ui.modalform.1"),
            Tr("grule.ui.modalform.2"),
            (pl, id1) => {
...
function Crash_Ui(pl) {
    ...
    fm.addDropdown(
        Tr("crash.drop"),
        onlinePlayers.map((p) => p.realName),
        0,
    );
    pl.sendForm(fm, (pl, data) => {
        if (data == null) return Close_Tell(pl);
    ...
    player.sendForm(form, (player, data) => {
        ...
        switch (data[2]) {
            case 0:
                /* ban */
                CmdEx(
                    (commands.Ban = _toSpace(
                        commands.Ban.replace("${Player}", selectedPlayer).replace("${time}", time).replace("${Reason}", reason),
                    )),
                );
                if (data[6] == 1) {
                    mc.broadcast(Tr("ban.broad", { name: selectedPlayer, reason: reason, time: time }));
                }
            ...
            case 0:
                ...
                function Forms_0(pl) {
                    ...
                    pl.sendForm(fm, (pl, dt) => {
                        if (dt == null) return Close_Tell(pl);
                        opl = pl.realName;
                        mc.getPlayer(Online_Players[dt[0]]).sendModalForm(
                            Tr("isForms.title", PLUGINS_NAME),
                            dt[1],
                            dt[2],
                            dt[3],
                            (pl, res) => {
                ...
                function Forms_1(pl) {
                    ...
                    pl.sendForm(fm, (pl, dt) => {
                        ...
                        function Mode0(pl) {
                            ...
                            pl.sendForm(fm, (pl, dt) => {
                                if (dt == null) {
                                    return mc.getPlayer(opl).tell(Gm_Tell + Tr("isForms.close", pl.realName));
                                }
                                if (dt[1] !== "") {
                                    return mc.getPlayer(opl).tell(Gm_Tell + Tr("isForms.Playerinput", pl.realName, dt[1]));
                                }
                            });
                        }
                    });
...
function Potion_Ui(pl) {
    let PotionData;
    if (!File.exists(Path + `Data\\PotionDatas.json`)) {
        File.writeTo(Path + `Data\\PotionDatas.json`, JSON.stringify(Config_init.PotionData, null, "\t"));
    }
    try {
        PotionData = JSON.parse(File.readFrom(Path + `Data\\PotionDatas.json`));
    } catch (e) {
        return logger.error(`Reasons <PotionData.json> file cannot be loaded: ${e}`);
    }
        ...
        switch (res) {
            case true:
                ...
                pl.sendForm(fm, (pl, dt) => {
                    ...
                    mc.getPlayer(Online_Players[dt[0]]).addEffect(
                        Number(PotionData[ID].type),
                        Number(dt[2]) * 20,
                        Number(dt[3]),
                        Boolean(dt[4]).valueOf(),
                    );
                    logger.debug(`effect "${Online_Players[dt[0]]}" ${PotionData[ID].type} ${dt[2]} ${dt[3]} ${dt[4]}`);
                });
                break;
            case false:
                pl.sendForm(fm, (pl, dt) => {
                    // 构建子表单 选择需要清除的药水
                    const fm = Func_Module.SimpleForm();
                    fm.setContent(Tr("potion.isdel"));
                    // 获取被清除药水的玩家对象
                    const gpl = mc.getPlayer(Online_Players[dt[0]]);
                    const potall = gpl.getAllEffects();
                    potall.forEach((i) => {
                        // 根据文件获取翻译数据及ID
                        const name = PotionData.findIndex((ii) => ii.type == i);
                        // 显示名称
                        fm.addButton(PotionData[name].name);
                    });
                    pl.sendForm(fm, (pl, id) => {
                        if (id == null) return Close_Tell(pl);
                        gpl.removeEffect(potall[id]);
                    });
                });
                break;
        }
...
function BindCmd() {
    ...
    BindCmd_Array.forEach((i) => {
        ...
        if (i.PluginVerification)
            CALLBACK_PROCESSING_CODE = String(
                `if (!Func_Module.qx(ori.player)) {if (!Permission_Group.USERS_GROUP(ori.player.xuid) == 1 || Permission_Group.USERS_GROUP(ori.player.xuid) == null) {return out.error(Gm_Tell + Tr("cmd.error"))};};`,
            );
        ...
        `
                .replace(/%Cmd%/g, ID)
                .replace(/%PermType%/g, CmdJurisdiction(i.PermType))
                .replace(/%CALLBACK_PROCESSING%/g, CALLBACK_PROCESSING_CODE),
        );
        logger.debug(StringCmd);
        try {
            eval(StringCmd);
        } catch (e) {
...
class Func_Module {
    ...
    static Write_Logs(module, trigger, content) {
        ...
        const logPath = Output_csv
            ? Date_Differentiation
                ? `.\\logs\\OPTools\\OPTools${time}.csv`
                : `.\\logs\\OPTools\\OPTools.csv`
            : Date_Differentiation
              ? `.\\logs\\OPTools\\OPTools${time}.log`
              : `.\\logs\\OPTools\\OPTools.log`;
