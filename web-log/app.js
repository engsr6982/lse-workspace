...
mc.listen("onServerStarted", () => {
    api = ll.hasExported("_CSV_JSON_", "_csvToJson_") ? ll.import("_CSV_JSON_", "_csvToJson_") : null;
});
        ...
        fs.createReadStream(filePath).on('data', chunk => {
            ...
            fs.createReadStream(filePath)
                ...
                .on('end', () => {
                    // eslint-disable-next-line no-console
                    console.log('\n');
                    logger.info(`处理文件完成  共：${rows.length} 行数据, 耗时: ${new Date() - start}ms`);
                    if (rows.length == 0) {
